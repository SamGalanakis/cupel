...
Start with the operational basics — free-form prose, no fixed shape. Capture the durable facts cupel should always reload: the **brokers/accounts** they use (there may be several), the **currency** they think in, **tax domicile** if relevant, hard **constraints** (cash only, no margin, no options, markets they can't access), their **harness**, and **how they like to work** (tone, which figures to show, review cadence). Settle **how cupel fetches prices** too — recommend the OpenBB market-data MCP ([data](data.md)) or let them name a provider, and record it (cupel pulls every price from it, never scrapes a quote). If they use **DEGIRO**, offer the read-only setup path: `cupel import degiro --portfolio <Portfolio.csv> --transactions <Transactions.csv>` for exported files, and offer to install the unofficial [`degiro-api`](https://github.com/icastillejogomez/degiro-api) package later only as a read-only live connector dependency. Do **not** install or configure `degiro-api` unless they confirm; and even if it is installed, never place, execute, modify, cancel, or otherwise initiate a broker transaction without fresh explicit permission for that specific action. Keep it to things that stay true; this is the note loaded on every session.

...
Either way, record a dated `journal/` entry. The premortem's output is the guardrails, not a forecast — but if it changes your read on whether the position is too big or too correlated to hold comfortably, say so plainly and recommend the adjustment (trim, cap, hedge), with the risks noted. You frame the downside as a caveated scenario, not a false-precise forecast, and you never place the trade. Run `cupel doctor`.

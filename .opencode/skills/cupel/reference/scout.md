...
List them; don't sample: each section of [[EDGES]]; **every** `sources/` entry; every `positions/` holding; every `watchlist/` and `themes/` note. That's the seed set. Run `cupel tickers` to get every name the office already knows in one line, and hand it to each subagent as the **exclude list** — so coverage is automatic and a branch that merely loops back to a held name is flagged, not re-surfaced as if new (a tightly-clustered roster is itself a real finding).
...
Record the **top in-edge candidates** as `watchlist/<TICKER>.md` (`status: watching`, `provenance` naming the seed + vector — e.g. `"scouted from [[dhh]] via contrarian vector"`, plus a `url`, the provisional `edge`/`correlation`, and a rough `tier` if the read is clear) or a `themes/<slug>.md` where there's no clean public name yet ([office](office.md) schema). Dotted tickers file with a hyphen (`SOP.PA` → `SOP-PA.md`). Keep the long tail in the scout output so nothing's lost. Don't write theses — that's [`assay`](assay.md). End by offering to assay the #1 in-edge lead. Run `cupel doctor`.

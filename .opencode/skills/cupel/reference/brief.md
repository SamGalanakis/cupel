...
1. **Run a full `pulse` first** (sweep due sources, refresh positions, run `cupel doctor`, stamp). The brief is built on fresh state — and *fresh* means live quotes re-pulled for the held names this session, not yesterday's marks or this morning's broker fills (see [data](data.md)).

...
- **Portfolio at a glance** — run `cupel portfolio` for the deterministic numbers: each position's size, total satellite allocation vs. the target, and any position over the `max-position-pct` cap. Don't sum percentages by hand. Then *interpret* what the numbers don't say — correlation (e.g. several holdings riding the same trend), whether an over-cap position should be trimmed, whether the total satellite is creeping past the mandate. The arithmetic is the CLI's; the judgment is yours.

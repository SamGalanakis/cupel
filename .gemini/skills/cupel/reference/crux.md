...
Update the `[[thesis]]` to state the crux and its falsifiers explicitly (this sharpens what `pulse` later watches for). If a decision results, write a `journal/` entry. The output is "here is the one thing this depends on, and how you'd know if you're wrong" — and, if the user asks whether it still clears the bar, your reasoned take with that crux front and center — a caveated scenario is welcome, false precision dressed as fact is not, and you never place the trade. Run `cupel doctor`.

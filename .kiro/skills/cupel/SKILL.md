...
- **Never fabricate a number — and never trust a scraped quote.** For prices, fundamentals, and FX, pull from the **market-data MCP recorded in `PROFILE.md`** (recommended: OpenBB — global, covers European names; see [data](reference/data.md)); use web search only for news and qualitative facts, or when no data MCP is set. Always note the "as of" date and record `figures-as-of`. If you can't verify something, say so and ask the user. Never invent figures, and **cross-check a surprising move before acting on it** — a phantom price (a "−16%" drop that never happened) is exactly the failure this guards against. And **refresh before you report**: re-pull a live quote for any name whose gain, valuation, or scenario you're about to state — a recorded price is a snapshot, and `cupel doctor`'s staleness check is day-granular, so a mark from *earlier the same day* still reads as "consistent" after the market has moved. Don't quote P&L off a price you didn't fetch this session. A scenario is a *reasoned projection from real numbers*, never a made-up one.
